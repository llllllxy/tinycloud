package org.tinycloud.common.utils.sm3;
...
public class SM3 {
    ...
    private static byte[] bigEndianIntToByte(int num) {
        return back(SM3Convert.intToBytes(num));
    }

    private static int bigEndianByteToInt(byte[] bytes) {
        return SM3Convert.byteToInt(back(bytes));
    }
    ...
    public static byte[] padding(byte[] in, int bLen) {
        ...
        pos += padd.length;
        byte[] tmp = back(SM3Convert.longToBytes(n));
